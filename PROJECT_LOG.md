...
Unit Tests:
  U1: ニトリURL → yamashita_test_nitori + utm_source=mcp_hub 付与 ✅
  U2: Amazon URL → tag=yamashita-22 + linkCode=as2 付与 ✅
...
Integration Tests:
  ...
  I2: ニトリ商品URLに yamashita_test_nitori が含まれる ✅
...
```json
{
  ...
  "platform": "nitori",
  "affiliate_url": "https://example.nitori.co.jp/products/n-click-cabinet-60?uid=yamashita_test_nitori&utm_source=mcp_hub&utm_medium=ai_agent&utm_campaign=mcp_hub_affiliate",
...
```json
{
  ...
  "product": {
    ...
    "price": 7990,
    "affiliate_url": "https://example.nitori.co.jp/...?uid=yamashita_test_nitori&..."
  },
  "affiliate_url": "https://example.nitori.co.jp/...?uid=yamashita_test_nitori&...",
...
#### 1. Amazon アソシエイト
- ID: `one20260410-22`（申請完了・審査中）
- `.env` の `AFFILIATE_ID_AMAZON` に反映済み
- 生成URL例: `https://www.amazon.co.jp/dp/...?tag=one20260410-22&linkCode=as2&...`

#### 2. 楽天アフィリエイト
- ID: `52b5b3f6.d649457f.52b5b3f7.c31ecd10`
...
- 生成URL例: `https://hb.afl.rakuten.co.jp/ichiba/52b5b3f6.d649457f.../`

#### 3. ハードコード除去
- コードベース全体から `yamashita` 文字列を完全除去
- テストコードも `.env` から動的にIDを読む形式に変更
- `grep -ri yamashita *.{ts,json,env,html}` → 0件
...
| プラットフォーム | ID | 状態 |
|---|---|---|
| Amazon | `one20260410-22` | 申請済み（審査中） |
| 楽天 | `52b5b3f6.d649457f...` | 本番ID反映済み |
...
- アプリID: `c6366df6-...` (API/バックエンドサービスとして登録)
- アクセスキー: 設定済み
- アフィリエイトID: `52b5d2c8.a6cc8790.52b5d2c9.8e1500db` (APIアプリ紐づき)
